...
public class ShiroConfig {
	 ...
	 * 因为authc是需要重新认证的, user表示用户不一定已Pass认证, 只要曾被Shiro记住过Log inStatus的用户就可以normal发起请求,比如rememberMe
	 * 以前的一个用户Log in时开启了rememberMe, 然后他关闭浏览器, 下次再访问时他就是一个user, 而不会authc
	 *
	...
	public ShiroFilterFactoryBean shirFilter(SecurityManager securityManager) {
		...
		// 用户，需要角色权限 “user”
		//filterChainDefinitionMap.put("/user/**", "roleOrFilter[家长|老师|Super Administrator]");
		// 这样写必须同时拥有两个角色才能访问
		// filterChainDefinitionMap.put("/user/**", "roles[\"组员, 管理员\"]");
		// 管理员，需要角色权限 “admin”
		//filterChainDefinitionMap.put("/sa/**", "roles[Super Administrator]");
		//filterChainDefinitionMap.put("/jz/**", "roleOrFilter[家长|教师|Super Administrator]");
		//filterChainDefinitionMap.put("/ls/**", "roleOrFilter[教师|Super Administrator]");
		// 开放登陆接口
