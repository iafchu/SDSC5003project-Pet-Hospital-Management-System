...
public class HealthController {
    ...
    public String applyListDoctor(Model model) {
        ...
        for(Pet p: rows){
            ...
            switch (sw){
                case 1:dsCount.add(p.getName() + "  keep your pet healthy");break;
                case 2:dsCount.add(p.getName() + "  petAbnormal请及时Treat!");break;
                case 3:dsCount.add(p.getName() + "  pet病情比较bad,请及时就医!");break;
                case 4:dsCount.add(p.getName() + "  很抱歉pet已无法治疗!");break;
                default:dsCount.add(p.getName() + "  pet基本健康,请继续保持");break;
            }
            ...
            if (t>=tsMin && t<=tsMax){
                tsCount.add("  temperaturenormal");
            }else if (t<tsMin){
                tsCount.add( "  temperaturelower");
            }else if (t>tsMax){
                tsCount.add( "  temperaturehigher");
            }

            if (w>=wsMin && w<=wsMax){
                wsCount.add( "  Weightnormal");
            }else if (w<wsMin){
                wsCount.add("  Weightlower");
            }else if (w>wsMax){
                wsCount.add("  Weighthigher");
            }

            if (h>=hsMin && h<=hsMax){
                hsCount.add("  Heightnormal");
            }else if (h<hsMin){
                hsCount.add( "  Heightlower");
            }else if (h>hsMax){
                hsCount.add("  Heighthigher");
            }

            if (a>=asMin && a<=asMax){
                asCount.add( "  food intakenormal");
            }else if (a<asMin){
                asCount.add("  food intakelower");
            }else if (a>asMax){
                asCount.add("  food intakehigher");
            }
        }
        model.addAttribute("pets", rows);
    ...
    /**
     * Ordinary userqueryrecords件数据BackPet log
     */
    ...
    /**
     * doctorqueryrecords件数据BackPet log
     */
