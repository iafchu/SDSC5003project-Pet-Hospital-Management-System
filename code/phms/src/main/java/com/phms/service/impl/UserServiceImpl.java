...
public class UserServiceImpl implements UserService {
	...
	public ResultMap login(String username, String password) {
		...
		if (!role.isEmpty()) {
			logger.info("欢迎Log in------您的权限是{}", role);
	...
	/**
	 *      Description: 检测用户旧password是否正确 <BR>
	 *      Remark: <BR>
	 * @param password
	 * @return <BR>
	 */
	...
	/**
	 *      Description: Change Password <BR>
	 *      Remark: <BR>
	 * @param password
	 * @return <BR>
	 */
	...
	public String updatePassword(String password) {
		...
		if (null != user) {
			...
			try {
				...
				logger.error("Change PasswordAbnormal", e);
				// 事物回滚
	...
	/**
	 *      Description: 根据用户idquery用户信息 <BR>
	 *      Remark: <BR>
	 * @param userId
	 * @return <BR>
	 */
	...
	public User selectUserByUserId(Long userId) {
		try {
			...
			logger.error("根据用户idquery用户信息Abnormal", e);
			return null;
		}
	}
	...
	public Object getAllUserByRoleId(Integer roleId, Integer page, Integer limit) {
		...
		try {
			...
			logger.error("根据权限id获取权限下所有用户Abnormal", e);
		}
	...
	public Object getAllUserByNotRoleId(Integer roleId, Integer page, Integer limit) {
		...
		try {
			...
			logger.error("根据权限id获取不是这个权限所有用户Abnormal", e);
		}
	...
	/**
	 *      Description: 根据records件query用户 <BR>
	 *      Remark: <BR>
	 * @param userParameter
	 * @return <BR>
	 */
	...
	public Object getAllUserByLimit(UserParameter userParameter) {
		...
		try {
			...
			logger.error("根据records件query用户 Abnormal", e);
		}
		MMGridPageVoBean<User> vo = new MMGridPageVoBean<>();
		vo.setTotal(size);
		vo.setRows(rows);
	...
	/**
	 *      Description: DELETE用户,并DELETE用户权限表 <BR>
	 *      Remark: <BR>
	 * @param id <BR>
	 */
	@Override
	...
	public void delUserById(Long id) {
		try {
			...
			userMapper.deleteByExample(userExample);// 更改用户Status
			userRoleMapper.deleteByExample(userRoleExample);// DELETE用户权限表
		} catch (Exception e) {
			logger.error("DELETE用户出现Abnormal", e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
		}
	}

	/**
	 *      Description: Add单个用户,抛出Abnormal <BR>
	 *      Remark: <BR>
	 * @param user
	 * @throws Exception <BR>
	 */
	...
	public String updateUser(User user) {
		try {
			...
			logger.error("根据用户id更新用户Abnormal", e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			return "ERR";
		}
	}
