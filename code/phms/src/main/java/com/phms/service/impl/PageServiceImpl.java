...
public class PageServiceImpl implements PageService {
	...
	public List<Page> getAllPageByRoleId(Integer roleId) {
		try {
			...
			logger.error("query角色下所有页面Abnormal", e);
	...
	public ResultMap delPageById(Integer id) {
		...
		try {
			...
			pageMapper.updateByExampleSelective(page, example);
			map.success().message("页面DELETE成功!");
		} catch (Exception e) {
			logger.error("页面DELETEAbnormal", e);
			map.fail().message("页面DELETE失败");
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
		}
		return map;
	}
	...
	public List<Page> getAllRolePageByUserId(String userId) {
		try {
			...
			logger.error("根据用户id获取页面Abnormal", e);
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			return Collections.emptyList();
		}
	}
