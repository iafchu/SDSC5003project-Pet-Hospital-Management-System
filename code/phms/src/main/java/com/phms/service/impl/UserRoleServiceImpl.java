...
public class UserRoleServiceImpl implements UserRoleService {
	...
	public List<UserRole> getRoleByUserId(String userId) {
		...
		try {
			...
			logger.error("根据用户id获取权限Abnormal", e);
			return Collections.emptyList();
		}
	}

	/**
	 *      Description: 根据权限idDELETE用户权限表 <BR>
	 *      Remark: <BR>
	 * @param id
	 * @return <BR>
	 */
	...
	public boolean delUserRoleByRoleId(int id) {
		...
		try {
			...
			logger.error("根据权限idDELETE用户权限表Abnormal", e);
			return false;
		}
	}
	...
	public ResultMap delUserRoleByUserIdAndRoleId(String userId, Integer roleId) {
		...
		if (userId.equals(user.getId())) {
			...
			try {
				userRoleMapper.deleteByExample(example);
				return new ResultMap().success().message("DELETE成功");
			} catch (Exception e) {
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				logger.error("根据用户id权限idDELETE用户权限表Abnormal", e);
				return new ResultMap().fail().message("DELETE失败");
			}
		}
	}
	...
	public String addUserRole(Integer roleId, String[] userIds) {
		try {
			...
			TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
			logger.error("添加用户角色 Abnormal", e);
			return "ERROR";
		}
	}
